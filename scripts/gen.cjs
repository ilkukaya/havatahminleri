const fs = require('fs');
const provinces = JSON.parse(fs.readFileSync('/home/user/havatahminleri/src/data/provinces.json','utf8'));
const provMap = {};
provinces.forEach(p => { provMap[p.plate] = p; });

function slug(s) {
  return s.toLowerCase().replace(/ğ/g,'g').replace(/ü/g,'u').replace(/ş/g,'s').replace(/ı/g,'i').replace(/ö/g,'o').replace(/ç/g,'c').replace(/â/g,'a').replace(/î/g,'i').replace(/û/g,'u').replace(/İ/g,'i').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'');
}

// All Turkish districts by plate code
const raw = {
...
9:["Bozdoğan","Buharkent","Çine","Didim","Efeler","Germencik","İncirliova","Karacasu","Koçarlı","Köşk","Kuyucak","Kuşadası","Nazilli","Söke","Sultanhisar","Yenipazar"],
...
14:["Dörtdivan","Göynük","Kıbrıscık","Mengen","Merkez","Mudurnu","Seben","Yeniçağa"],
...
for (const [plateStr, names] of Object.entries(raw)) {
  const plate = parseInt(plateStr);
  const prov = provMap[plate];
  if (!prov) { console.error('Missing province for plate', plate); continue; }
  
  for (const name of names) {
    let s = slug(name);
    
    // Handle Merkez specially
    if (name === 'Merkez') {
      s = prov.slug + '-merkez';
    }
    
    // Handle duplicate slugs across provinces
    if (usedSlugs.has(s)) {
      s = s + '-' + prov.slug;
    }
    
    // Deduplicate within same province
    const key = prov.slug + '/' + s;
    if (usedSlugs.has(key)) continue;
    usedSlugs.add(key);
    usedSlugs.add(s);
    
    // Generate approximate coordinates - offset from province center
    const offset = () => (Math.random() - 0.5) * 0.5;
    const lat = Math.round((prov.lat + offset()) * 10000) / 10000;
    const lon = Math.round((prov.lon + offset()) * 10000) / 10000;
    
    result.push({ name, slug: s, province: prov.slug, lat, lon });
  }
}

// Sort by province then name
result.sort((a, b) => a.province.localeCompare(b.province) || a.name.localeCompare(b.name));

fs.writeFileSync('/home/user/havatahminleri/src/data/districts.json', JSON.stringify(result, null, 2));
console.log('Total districts:', result.length);
console.log('Provinces covered:', new Set(result.map(d => d.province)).size);
